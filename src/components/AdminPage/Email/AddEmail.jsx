...
function AddEmail() {
    ...
    const [categorySelect, setcategorySelect] = useState(EmailCategory[0] ? EmailCategory[0].slug : "")

    const [editorState, setEditorState] = useState(EmailData[categorySelect] ? EmailData[categorySelect] : "")
    ...
    const saveEmail = () => {
        ...
        set(ref(db, 'EmailData/' + categorySelect), editorState)
    ...
    useEffect(() => {
        setEditorState(EmailData[categorySelect] ? EmailData[categorySelect] : "")
    }, [categorySelect])
        ...
        <div className="flex w-full md:p-8 items-center justify-center">
            <div className="flex flex-col w-full">
                    ...
                    >
                        {
                            EmailCategory.map(EmailItem => (
                                <option key={Math.random()} value={EmailItem.slug}>{EmailItem.slug}</option>
                            ))
                        }
                    </select>
                </div>
                ...
                <div className="mb-4 w-full mt-2">
                    <button onClick={saveEmail} className="mt-4 px-4 py-2 rounded text-white inline-block shadow-lg bg-blue-500 hover:bg-blue-600 focus:bg-blue-700" type="submit">Save</button>
                </div>
            </div>
        </div>
    )
